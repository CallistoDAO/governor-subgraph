...
describe("delegation", () => {
  beforeEach(() => {
    ...
    createMockedFunction(
      Address.fromString(GOHM_CONTRACT),
      "balanceOf",
  ...
  test("initial delegation", () => {
    ...
    assert.entityCount(
      "DelegateChanged",
      1,
      "DelegateChanged: Should be 1 DelegateChanged entity",
    );
    const delegateChangedRecord = DelegateChanged.load(
      HOLDER.concatI32(DEFAULT_BLOCK_NUMBER.toI32()).concatI32(
    ...
    assert.entityCount(
      "VoteDelegator",
      1,
      "VoteDelegator: Should be 1 VoteDelegator entity",
    );
    ...
    assert.entityCount(
      "VoterVotingPowerSnapshot",
      1,
      "VoterVotingPowerSnapshot: Should be 1 VoterVotingPowerSnapshot entity",
    );
    const voterVotingPowerSnapshotRecord = VoterVotingPowerSnapshot.load(
      VOTER.concatI32(DEFAULT_BLOCK_NUMBER.toI32()).concatI32(
        DEFAULT_LOG_INDEX.toI32(),
      ),
    );
    assert.stringEquals(
      voterVotingPowerSnapshotRecord!.voter.toHexString(),
      VOTER.toHexString(),
      "VoterVotingPowerSnapshot: Voter should be the voter",
    );
    assert.stringEquals(
      voterVotingPowerSnapshotRecord!.votingPower.toString(),
      GOHM_BALANCE.toString(),
      "VoterVotingPowerSnapshot: Voting power should be the gOHM balance",
    );
    ...
    assert.stringEquals(
      voterRecord!.address.toHexString(),
      VOTER.toHexString(),
  ...
  test("delegatee changed", () => {
    ...
    const newVoterRecord = Voter.load(
      Address.fromString("0x0000000000000000000000000000000000000003"),
    );
    assert.stringEquals(
      newVoterRecord!.address.toHexString(),
      Address.fromString(
        "0x0000000000000000000000000000000000000003",
      ).toHexString(),
      "Voter: Address should be the new delegatee",
    );
    ...
    assert.entityCount(
      "DelegateChanged",
      2,
      "DelegateChanged: Should be 2 DelegateChanged entities",
    );
    const delegateChangedRecord = DelegateChanged.load(
      HOLDER.concatI32(DEFAULT_BLOCK_NUMBER.toI32()).concatI32(
        newLogIndex.toI32(),
    ...
    assert.stringEquals(
      ...
      "DelegateChanged: New delegatee should be the new delegatee",
    );

    // Validate VoterVotingPowerSnapshot for the old delegatee
    const previousDelegateeVotingPowerSnapshotRecord =
      VoterVotingPowerSnapshot.load(
        VOTER.concatI32(DEFAULT_BLOCK_NUMBER.toI32()).concatI32(
          newLogIndex.toI32(),
        ),
      );
    assert.stringEquals(
      previousDelegateeVotingPowerSnapshotRecord!.votingPower.toString(),
      "0",
      "VoterVotingPowerSnapshot: Voting power of previous delegatee should be 0",
    );

    // Validate VoterVotingPowerSnapshot for the new delegatee
    const newDelegateeVotingPowerSnapshotRecord = VoterVotingPowerSnapshot.load(
      Address.fromString("0x0000000000000000000000000000000000000003")
        .concatI32(DEFAULT_BLOCK_NUMBER.toI32())
        .concatI32(newLogIndex.toI32()),
    );
    assert.stringEquals(
      newDelegateeVotingPowerSnapshotRecord!.votingPower.toString(),
      GOHM_BALANCE.toString(),
      "VoterVotingPowerSnapshot: Voting power of new delegatee should be the gOHM balance",
    );
  });
  ...
  test("transfer with recipient not delegated", () => {
    ...
    assert.assertNull(
      newDelegateeVotingPowerSnapshotRecord,
      "VoterVotingPowerSnapshot: New delegatee should not have a voting power snapshot",
    );
  });
});
